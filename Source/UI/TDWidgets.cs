...
namespace TD_Find_Lib
{
	static class TDWidgets
	{
		...
		/*
		public static bool FloatRange(Rect rect, int id, ref FloatRangeUB rangeUB, float min = 0f, float max = 1f, string labelKey = null, ToStringStyle valueStyle = ToStringStyle.FloatTwo, float gap = 0f, GameFont sliderLabelFont = GameFont.Tiny, Color? sliderLabelColor = null)
		{
			FloatRange newRange = rangeUB.range;
			Widgets.FloatRange(rect, id, ref newRange, min, max, labelKey, valueStyle, gap, sliderLabelFont, sliderLabelColor);
			if (rangeUB.range != newRange)
			{
				rangeUB.range = newRange;
				return true;
			}
			return false;
		}
